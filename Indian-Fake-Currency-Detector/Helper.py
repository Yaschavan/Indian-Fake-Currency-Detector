...
OLD_NOTE_TYPES = {10, 20, 50, 100}
NEW_NOTE_TYPES = {200, 500}
SUPPORTED_NOTE_TYPES = [10, 20, 50, 100, 200, 500]
...
def get_reference_image(note_type, feature_index):
    reference_path = os.path.join(IMAGE_DIR, str(note_type), f"000{feature_index + 1}.jpg")
    return cv2.imread(reference_path, 0)

# take image input
def takeImageInput(_path):
...
def verify_features(proper_image, note_type, feature_set_data, verify_indices, calculate_fn, missing_value=65):
    accuracy_values = np.full(len(verify_indices), missing_value, dtype=float)

    for idx, feature_index in enumerate(verify_indices):
        sample = getFeatureImage(proper_image, feature_set_data[feature_index])
        reference_image = get_reference_image(note_type, feature_index)

        if reference_image is None or sample.size == 0:
            continue

        try:
            matches, kp = imageMatcher(reference_image, sample)
            if matches is None or kp is None:
                continue
            accuracy_values[idx] = determineAccuracy(matches)
        except Exception:
            pass

    return calculate_fn(accuracy_values)[0]
...
def prepare_note_image(image_path):
    coloredImage, grayscale = takeImageInput(image_path)

    if coloredImage is None or grayscale is None:
        return None
    ...
    return getFit(coloredImage, rect)


def identify_note_type_from_features(proper_image, note_type, feature_set_data, detect_indices, missing_value=100):
    distances = []

    for feature_index in detect_indices:
        sample = getFeatureImage(proper_image, feature_set_data[feature_index])
        reference_image = get_reference_image(note_type, feature_index)

        if reference_image is None or sample.size == 0:
            distances.append(missing_value)
            continue
        ...
        try:
            ...
            if matches is None or kp is None:
                distances.append(missing_value)
                continue
            distances.append(determineAccuracy(matches))
        except Exception:
            distances.append(missing_value)
...
def identify_note_type(proper_image):
    best_note_type = None
    best_score = float("inf")

    for note_type in SUPPORTED_NOTE_TYPES:
        if note_type in NEW_NOTE_TYPES:
            score = identify_note_type_from_features(proper_image, note_type, feature_set, detect_feat)
        else:
            score = identify_note_type_from_features(proper_image, note_type, feature_set_old, detect_feat_old)

        if score < best_score:
            best_score = score
            best_note_type = note_type

    return best_note_type


def analyze_note_image(image_path, option=None):
    proper_image = prepare_note_image(image_path)

    if proper_image is None:
        return {"score": 0.0, "note_type": None}

    if option is not None:
        Note_Type = int(option)
    else:
        Note_Type = identify_note_type(proper_image)

    if Note_Type is None:
        return {"score": 0.0, "note_type": None}

    if Note_Type in NEW_NOTE_TYPES:
        score = verifynewnotes(None, None, Note_Type, proper_image)
    elif Note_Type in OLD_NOTE_TYPES:
        score = verifyoldnotes(None, None, Note_Type, proper_image)
    else:
        score = 0.0

    return {"score": score, "note_type": str(Note_Type)}


def dFinal(option):
    root = "static/Output/"
    fileList = sorted(getFiles(root))

    if not fileList:
        return 0.0

    image_path = os.path.join(root, fileList[0])
    return analyze_note_image(image_path, option)["score"]
